...
// This file generates an optimized cut of the data stored in /pid_files/VarSpread.root  
#include "TCut.h"
#include "TH1D.h"
#include "TAxis.h"
#include "string"
...
#define BIN_NUM 50
#define MAX 1.4
void LArPIDCutSpread()
{

  TFile* f = new TFile("pid_files/VarSpread.root");
  TTree* t = (TTree*)f->Get("SpreadTree");
  TH1D* bg = new TH1D("bg","Spread Variable", BIN_NUM,0,MAX);
  TH1D* sig = new TH1D("sig","Signal",BIN_NUM,0,MAX);
  bg->GetXaxis()->SetTitle("Spread");
  ...
  Double_t bg_h, sig_h;
  t->SetBranchAddress("spread_bg",&bg_h);
  t->SetBranchAddress("spread_signal",&sig_h);
  ...
  Double_t sig_cont, bg_cont;
      for ( int l = BIN_NUM; l>1; l--)
	{
	  for (int m=l; m<BIN_NUM; m++)
	    {
  ...
  TH1D* bg_cut = new TH1D("bg_cut","Post-Cut Spread Variable",BIN_NUM,0,MAX);
  TH1D* sig_cut = new TH1D("sig_cut","Signal Spread",BIN_NUM,0,MAX);
  bg_cut->GetXaxis()->SetTitle("Spread");
  bg_cut->GetYaxis()->SetTitle("Arbitrary Units");
  bg_cut->SetStats(0);
  bg_cut->SetMaximum(2000);
  ...
  TCanvas *mycan = new TCanvas("mycan","Spread Cut", 1600,100,1500,500);
