// An initial check for the validity of the TPrincipal PCA output, using a ratio of the first two eigenvalues as a metric, and plotting against momentum.
...
Double_t EigenMet( Double_t Eigen_1, Double_t Eigen_2 ) 
{ 
  if ( ( (Eigen_2/Eigen_1) < 1) && ( (Eigen_2/Eigen_1) > 0) ) 
    {return Eigen_2/Eigen_1; }
  else {return 0; }
}

Double_t MomMag( Double_t px, Double_t py, Double_t pz) 
{ 
  Double_t mag = sqrt( ((px*px)) + ((py*py)) + ((pz*pz)) );
  return mag;
...
void LArPIDCheck1() 
{
  ...
  TFile* HistFile = new TFile("pid_files/MomVEigenHist.root","RECREATE");
...
for(int i=0;i<nentries_muon;++i)
  {
    ...
    if ( pid_muon->EigenValues.size() && ana_muon->NHits )
      {
	Eig_Met_Con_muon[i] = EigenMet(pid_muon->EigenValues[0][0],pid_muon->EigenValues[0][1]);
	Mom_Con_muon[i] = MomMag(ana_muon->TrajStart4MomMC[0].X(), ana_muon->TrajStart4MomMC[0].Y(), ana_muon->TrajStart4MomMC[0].Z());
 ...
 for(int j=0;j<nentries_electron;++j)
   {
     ...
     if ( pid_electron->EigenValues.size() && ana_electron->NHits )
       {
	 Eig_Met_Con_electron[j] = EigenMet(pid_electron->EigenValues[0][0],pid_electron->EigenValues[0][1]);
	 Mom_Con_electron[j] = MomMag(ana_electron->TrajStart4MomMC[0].X(), ana_electron->TrajStart4MomMC[0].Y(),
				      ana_electron->TrajStart4MomMC[0].Z());
 ...
 for(int i=0;i<nentries_proton;++i)
   {
     ...
     if ( pid_proton->EigenValues.size() && ana_proton->NHits )
       {
	 Eig_Met_Con_proton[i] = EigenMet(pid_proton->EigenValues[0][0],pid_proton->EigenValues[0][1]);
	 Mom_Con_proton[i] = MomMag(ana_proton->TrajStart4MomMC[0].X(), ana_proton->TrajStart4MomMC[0].Y(), ana_proton->TrajStart4MomMC[0].Z());
 ...
 for(int i=0;i<nentries_piplus;++i)
   {
     ...
     if ( pid_piplus->EigenValues.size() && ana_piplus->NHits )
       {
	 Eig_Met_Con_piplus[i] = EigenMet(pid_piplus->EigenValues[0][0],pid_piplus->EigenValues[0][1]);
	 Mom_Con_piplus[i] = MomMag(ana_piplus->TrajStart4MomMC[0].X(), ana_piplus->TrajStart4MomMC[0].Y(), ana_piplus->TrajStart4MomMC[0].Z());
