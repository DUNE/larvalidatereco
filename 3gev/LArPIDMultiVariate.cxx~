...
void LArPIDMultiVariate(Int_t ntrain = 100)
{
  ...
  con_tree->SetDirectory(0);
  grad_tree->SetDirectory(0);
  spread_tree->SetDirectory(0);
  // Create new combined tree
  TTree* nn_tree = new TTree("nn_tree","Input Tree");
  Double_t con_cont, grad_cont, spread_cont;
  Int_t type;

  TBranch* con_bg = con_tree->GetBranch("con_bg");
  TBranch* con_sig = con_tree->GetBranch("con_sig");
  TBranch* grad_bg = grad_tree->GetBranch("grad_bg");
  TBranch* grad_sig = grad_tree->GetBranch("grad_sig");
  TBranch* spread_bg = spread_tree->GetBranch("spread_bg");
  TBranch* spread_sig = spread_tree->GetBranch("spread_sig");
  ...
  for ( i = 0 ; i < spread_tree->GetEntries(); i++ )
    {
      spread_cont = spread_sig->GetEntry(i);
  ...
  for ( i = 0; i < spread_tree->GetEntries(); i++ )
    {
      spread_cont = spread_bg->GetEntry(i);
      //spread_cont = spread_tree->GetEntry(i);
      nn_tree->Fill();
    }
  type=1;
