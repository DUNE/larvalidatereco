...
// PID variable to compute the gradient of the SD of the distance of hits from the principal axis as a function of the longitudinal position along the track
...
void LArPIDVarGradAll()

{

  TFile* GradFile = new TFile("pid_files/VarGradAll.root","RECREATE");
  TH1D* hist_proton_grad = new TH1D("hist_proton_grad","Proton",20,0,20);
  TH1D* hist_piplus_grad = new TH1D("hist_piplus_grad", "Piplus",20,0,20);
  TH1D* hist_signal_grad = new TH1D("hist_signal_grad","Signal",20,0,20);

  Double_t grad_proton, grad_piplus, grad_signal;
  ...
  // Fill vectors 
  std::vector<std::vector<TVector3>> trackcon_proton = FillTracksGrad("/data/t2k/phumhh/lbne/ntuple_3gev_proton.root");
  std::vector<std::vector<TVector3>> trackcon_piplus = FillTracksGrad("/data/t2k/phumhh/lbne/ntuple_3gev_piplus.root");
  std::vector<std::vector<TVector3>> trackcon_electron = FillTracksGrad("/data/t2k/phumhh/lbne/ntuple_3gev_electron_tracks.root");


  std::vector<Double_t>* grad_hold_proton = new std::vector<Double_t>;
  std::vector<Double_t>* grad_hold_piplus = new std::vector<Double_t>;
  ...
  hist_proton_grad->Fill(sd_grad_proton);
  grad_hold_proton->push_back(sd_grad_proton);
  ...
  for (auto i = trackcon_piplus.begin(); i!=trackcon_piplus.end(); i++ )
      ...
      for (Double_t slice=0; slice!= slice_num; slice++)
	{
	  ...
	  // ADD SD TO NTUPLE HERE
	  hist_piplus_grad->Fill(sd_grad_piplus);
	  grad_hold_piplus->push_back(sd_grad_piplus);
	  slice_sd_prev = slice_sd;
	  hit_rad->clear();
	  hit_tot = 0; slice_sd_prev = 0; slide_sq_tot = 0;
	}
  ...
  VarTree->Branch("grad_proton", &grad_proton, "grad_proton/D");
  VarTree->Branch("grad_signal", &grad_signal, "grad_signal/D");
  VarTree->Branch("grad_piplus",&grad_piplus,"grad_piplus/D");
  Int_t nentries_fill;
  if(grad_hold_proton->size()>grad_hold_electron->size() && grad_hold_piplus->size()>grad_hold_electron->size()) { nentries_fill =  grad_hold_electron->size(); }
  else { nentries_fill = grad_hold_proton->size(); }

  for (int i = 0; i<nentries_fill; i++ )
    {
      if (grad_hold_proton->at(i) && grad_hold_piplus->at(i) && grad_hold_electron->at(i) ) 
	{
	  grad_signal = grad_hold_electron->at(i);
	  grad_proton = grad_hold_proton->at(i);
	  grad_piplus = grad_hold_piplus->at(i);
	  VarTree->Fill();
	}
    }
  GradFile->cd();
  ...
  hist_signal_grad->Scale(1/hist_signal_grad->Integral());
  hist_proton_grad->Scale(1/hist_proton_grad->Integral());
  hist_piplus_grad->Scale(1/hist_piplus_grad->Integral());
  ...
  myleg->AddEntry(hist_proton_grad);
  myleg->AddEntry(hist_piplus_grad);
  hist_proton_grad->SetFillColor(kBlue);
  hist_signal_grad->SetFillColor(kRed);
  hist_piplus_grad->SetFillColor(kGreen);
  hist_proton_grad->SetFillStyle(3004);
  hist_proton_grad->SetLineColor(kBlue);
  hist_piplus_grad->SetFillStyle(3005);
  hist_piplus_grad->SetLineColor(kGreen);
  hist_signal_grad->SetFillStyle(3001);
  hist_signal_grad->SetLineColor(kRed);
  hist_signal_grad->SetMaximum(1);
  hist_signal_grad->GetXaxis()->SetTitle("Gradient Variable");
  Double_t norm = hist_proton_grad->Integral() + hist_piplus_grad->Integral() + hist_signal_grad->Integral();
  hist_proton_grad->Scale(1/norm);
  hist_piplus_grad->Scale(1/norm);
  hist_signal_grad->Scale(1/norm);
  hist_proton_grad->SetDirectory(0);
  hist_piplus_grad->SetDirectory(0);
  hist_signal_grad->SetDirectory(0);
  hist_signal_grad->Draw();
  hist_proton_grad->Draw("SAME");
  hist_piplus_grad->Draw("SAME");
  myleg->Draw();
  hist_proton_grad->Write();
  hist_piplus_grad->Write();
