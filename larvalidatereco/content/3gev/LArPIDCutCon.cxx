// This file generates an optimized cut of the data stored in /pid_files/VarCon.root
...
void LArPIDCutCon()
{
  ...
  TFile* f = new TFile("pid_files/VarCon.root");
  TTree* t = (TTree*)f->Get("ConTree");
  TH1D* bg = new TH1D("bg","Con Variable", BIN_NUM,0,MAX);
  TH1D* sig = new TH1D("sig","Signal",BIN_NUM,0,MAX);
  bg->GetXaxis()->SetTitle("Conicalness");
  ...
  Double_t bg_h, sig_h;
  t->SetBranchAddress("con_bg",&bg_h);
  t->SetBranchAddress("con_signal",&sig_h);
  ...
  // Compute optimal cut by taking the difference between signal and bg bin height, then dividing by the number of entrie sin the proposed cut.
  ...
  Double_t sig_cont, bg_cont;
      for ( int l = 0; l<BIN_NUM; l++)
	{
	  ...
	  //sig_tot = sig->Integral(1,l);
	  //bg_tot = bg->Integral(1,l);
	  if ( sig->GetBinContent(l) > bg->GetBinContent(l) && l>25) { diff[l] = (sig_tot-bg_tot)/(sig->Integral(l,BIN_NUM)+bg->Integral(l,BIN_NUM));}
	  else { diff[l] = 0; 
	  }
	  if (bg->GetBinContent(l) < sig->GetBinContent(l) && l>25) {
	    diff_err[l] =
	      std::sqrt(
	      TMath::Power
		      (
  ...
  TH1D* bg_cut = new TH1D("bg_cut","Post-Cut Conicalness Variable",BIN_NUM,0,MAX);
  TH1D* sig_cut = new TH1D("sig_cut","Signal Con",BIN_NUM,0,MAX);
  bg_cut->GetXaxis()->SetTitle("Conicalness");
